...
namespace S5FS
{
    ...
    internal class SuperBlock
    {
        ...
        const int max_inode_list_size = 250;
        ...
        /// <summary>
        /// Номер последнего блока, занятого битовой картой. 8 байт.
        /// </summary>
        public UInt64 s_lbit;
        /// <summary>
        /// Список номеров свободных inode PS: постоянного размера.
        /// 2000 байт (250 адресов).
        /// </summary>
        public UInt64[] s_f_inodes;
        ...
        public SuperBlock(UInt64 disk_size, UInt32 s_blen)
        {
            ...
            //Нужно определить минимальный размер диска, чтобы нужно было хотя бы в 2 раза больше инодов
            this.s_f_inodes = new UInt64[max_inode_list_size];
            //0й инод - корневой
            for (UInt64 i = 0; i < (UInt64)s_f_inodes.Length; i++)
            {
                s_f_inodes[i] = i + 1;
            }
        }
